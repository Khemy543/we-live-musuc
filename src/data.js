import React from "react";

const data = [
    {
        id:1,
        name:"Roosevelt Ansa- Asare (lovaboi)",
        profile:require("assets/img/Lovaboi/profile.jpeg"),
        music_type:" Afrobeats Recording Artist",
        user:"rapper, singer and songwriter",
        no_of_tracks:4,
        about:" I'm a simple but smart guy with an amazing voice, I rap, sing and also a songwriter. My mentor is wizkid. I love all genre of music.",
        contact:{
            facebook:"https://web.facebook.com/roosevelt.ansaasare",
            instagram:"https://www.instagram.com/_lovaboi_/",
            twitter:"https://twitter.com/_lovaboi_",
            email:"rooseveltansaasare.raa@gmail.com"
        },
        images:[
            {
                id:1,
                src:require("assets/img/Lovaboi/image1.jpeg"),
                caption:"LovaBoi",
                altText:""
            },
            {
                id:2,
                src:require("assets/img/Lovaboi/image2.jpeg"),
                caption:"LovaBoi",
                altText:""
            },
            {
                id:3,
                src:require("assets/img/Lovaboi/image3.jpeg"),
                caption:"LovaBoi",
                altText:""
            }
        ],
        music:[{
            id:1,
            name:"BIEM!", 
            artists:"Kalipharma Ft Lovaboi x Nah-nah",
            link:[
                {
                    name:"Audiomack",
                    link:"https://audiomack.com/lucky-gent/song/biem",
                }
            ],
            cover:require("assets/img/Lovaboi/Music/biem.jpeg"),
            released:"30 August 2019"
        },
        {
            id:2,
            name:"Low Tempo",
            artists:"Lovaboi x Neon",
            link:[
                {
                    name:"Audiomack",
                    link:"https://audiomack.com/lucky-gent/song/low-tempo"
                }
            ],
            cover:require("assets/img/Lovaboi/Music/lovetempo.jpeg"),
            released:"30 August 2019"
        },
        {
            id:3,
            name:"Give me wine",
            artists:"Lucky Gent",
            link:[
                {
                    name:"Audiomack",
                    link:"https://audiomack.com/lucky-gent/song/give-me-wine",
                }
            ],
            cover:require("assets/img/Lovaboi/Music/givemewine.jpeg"),
            released:"30 August 2019"
        },
        {
            id:4,
            name:"To the club",
            artists:"Munira x Boi_Nah_Nah x Lovaboi",
            link:[
                {
                    name:"Sound Cloud",
                    link:"https://soundcloud.com/user-642014821/to-the-club",
                }
            ],
            cover:require("assets/img/Lovaboi/Music/totheclub.jpeg"),
            released:"30 August 2019"
        }]
    },
     {
        id:2,
        name:"Adolf C Pacman",
        profile:require("assets/img/Adolf/profile.jpeg"),
        music_type:" AfroReggaeDancehall Artise",
        user:"Affro Reggae Dancehall",
        no_of_tracks:0,
        about:" A simple Ghanaian youth brethren trying to make ends meet like everyone. One happy soul that finds fun in entertaining..",
        contact:{
            facebook:"https://mobile.facebook.com/timewillcomenow?ref=bookmarks&_rdc=1&_rdr#_=_ ",
            instagram:"https://www.instagram.com/pac.adolf/", 
            twitter:"https://twitter.com/AdolfPacman",
            email:""
        },
        images:[
            {
                id:1,
                src:require("assets/img/Adolf/image1.jpeg"),
                caption:"Adolf",
                altText:""
            },
            {
                id:2, 
                src:require("assets/img/Adolf/image2.jpeg"),
                caption:"Adolf",
                altText:""
            },
            {
                id:3,
                src:require("assets/img/Adolf/image3.jpeg"),
                caption:"Adolf",
                altText:""
            }
        ],
        music:[]
    },
    {
        id:3,
        name:"Nana Kwame Boateng (ZMOKE)",
        profile:require("assets/img/Zmoke/profile.jpeg"),
        music_type:"Music Producer for All Genre",
        user:"Music Producer ",
        no_of_tracks:1,
        about:" Music producer for all genre of music. Born on 22 November 1997. My quote for music is always going to be 'music for the masters' ",
        contact:{
            facebook:"",
            instagram:"https://www.instagram.com/zmoke_1/",
            twitter:"",
            email:"www.beatstars.com/zmoke"
        },
        images:[
            {
                id:1,
                src:require("assets/img/Zmoke/image1.jpeg"),
                caption:"Zmoke",
                altText:""
            },
            {
                id:2,
                src:require("assets/img/Zmoke/image2.jpeg"),
                caption:"Zmoke",
                altText:""
            },
            {
                id:3,
                src:require("assets/img/Zmoke/image3.jpeg"),
                caption:"Zmoke",
                altText:""
            },
            {
                id:4,
                src:require("assets/img/Zmoke/image4.jpeg"),
                caption:"Zmoke",
                altText:""
            }
        ],
        music:[{
            id:1,
            name:"Fine LADY (Afro Dancehall beat)", 
            artists:"Beats By Zmoke",
            link:[
                {
                    name:"Beatstars",
                    link:"https://www.beatstars.com/beat/fine-lady--afro-dancehall-beat-4843490",
                }
            ],
            cover:require("assets/img/Zmoke/music/fine_lady.png"),
            released:"3 January 2020"
        },
        ]
    },
    {
        id:4,
        name:"Kalipharma",
        profile:require("assets/img/Kalipharma/profile.jpeg"),
        music_type:"Afro pop and trap music",
        user:"Change EP (T.E.C.H)",
        no_of_tracks:5,
        about:"Short info about artist:  A recording artist with a 5 track EP. Nominated for Cbaze awards 19 as the student rapper of the year.",
        contact:{
            facebook:"",
            instagram:"",
            twitter:"https://twitter.com/Gootherre",
            email:""
        },
        images:[
            {
                id:1,
                src:require("assets/img/Kalipharma/image3.jpeg"),
                caption:"Kalipharma",
                altText:""
            },
            {
                id:2,
                src:require("assets/img/Kalipharma/image2.jpeg"),
                caption:"Kalipharma",
                altText:""
            },
            {
                id:3,
                src:require("assets/img/Kalipharma/image1.jpeg"),
                caption:"Kalipharma",
                altText:""
            }
        ],
        music:[{
            id:1,
            name:"BIEM!", 
            artists:"Kalipharma Ft Lovaboi x Nah-nah",
            link:[
                {
                    name:"Audiomack",
                    link:"https://audiomack.com/lucky-gent/song/biem",
                }
            ],
            cover:require("assets/img/Lovaboi/Music/biem.jpeg"),
            released:"30 August 2019"
        },
        {
            id:2,
            name:"CHANGE EP (T.E.C.H)",
            artists:"Kalipharma",
            link:[
                {
                    name:"Audiomack",
                    link:"https://audiomack.com/go-there/album/change-ep-1"
                }
            ],
            cover:require("assets/img/Kalipharma/tech.jpeg"),
            released:"30 August 2019"
        }]
    },
    {
        id:5,
        name:"Lordmansfield Asante (Oh Lord) ",
        profile:require("assets/img/OhLord/profile.jpeg"),
        music_type:" Afrobeats Recording Artist",
        user:"rapper, singer and songwriter",
        no_of_tracks:0,
        about:" A simple Ghanaian youth brethren trying to make ends meet like everyone. One happy soul that finds fun in entertaining.",
        contact:{
            facebook:"",
            instagram:"",
            twitter:"",
            email:""
        },
        images:[
            {
                id:1,
                src:require("assets/img/OhLord/image1.jpeg"),
                caption:"Oh Lord",
                altText:""
            },
            {
                id:2,
                src:require("assets/img/OhLord/image2.jpeg"),
                caption:"Oh Lord",
                altText:""
            },
            {
                id:3,
                src:require("assets/img/OhLord/image3.jpeg"),
                caption:"Oh Lord",
                altText:""
            }
        ],
        music:[]
    },
     {
        id:6,
        name:"Bishop",
        profile:require("assets/img/bishop/profile.jpeg"),
        music_type:" Afrobeats Recording Artist",
        user: "A rapper with melodies. Imagine drake, Cole, polo G and roddy ricch being one person on a Kanye instrumental.",
        no_of_tracks:0,
        about:" A youngin trying to better his life and that of Africa. A pressure cooker who resonates positive energy. Modern day Robin Hood.A rapper with melodies. Imagine drake, Cole, polo G and roddy ricch being one person on a Kanye instrumental.",
        contact:{
            facebook:"",
            instagram:"https://www.instagram.com/bishop_1one/",
            twitter:"https://twitter.com/BISHOP1one",
            email:""
        },
        images:[ 
            /* {
                id:1,
                src:require("assets/img/bishop/image1.jpeg"),
                caption:"Bishop",
                altText:""
            }, */
            {
                id:1,
                src:require("assets/img/bishop/image2.jpeg"),
                caption:"Bishop",
                altText:""
            }
        ],
        music:[]
    },
    {
        id:7,
        name:"Boi Nah-Nah",
        profile:require("assets/img/boiNana/profile.jpeg"),
        music_type:" Rnb and Afro dancehall artiste ",
        user:"good music for the funs ",
        no_of_tracks:0,
        about:" Ghanaian based musician inspired by the comfort of music and dedicated to Entertain and  hit your play list with good music for the funs .",
        contact:{
            facebook:"https://web.facebook.com/Boi-Nah-Nah-Music-104236060947677",
            instagram:"https://www.instagram.com/boi_nah_nah/",
            twitter:"https://twitter.com/BoiNahNah1",
            email:""
        },
        images:[
            {
                id:1,
                src:require("assets/img/boiNana/image1.jpeg"),
                caption:"Boi Nah nah",
                altText:""
            },
            {
                id:2,
                src:require("assets/img/boiNana/image2.jpeg"),
                caption:"Boi Nah nah",
                altText:""
            },
            {
                id:3,
                src:require("assets/img/boiNana/image3.jpeg"),
                caption:"Boi Nah nah",
                altText:""
            }
        ],
        music:[]
    },
    {
        id:8,
        name:"Gilbert Asare( Kay B)",
        profile:require("assets/img/kayb/profile.jpeg"),
        music_type:" Singer of All genre ",
        user:" Music is the  food  for the soul .",
        no_of_tracks:0,
        about:"Am a Ghanaian based singer of all genre. Born on 12 April 1996.",
        contact:{
            facebook:"https://web.facebook.com/profile.php?id=100012194168219",
            instagram:"https://www.instagram.com/kofibista/",
            twitter:"https://twitter.com/BistaKofi",
            email:""
        },
        images:[
            {
                id:1,
                src:require("assets/img/kayb/image1.jpeg"),
                caption:"KayB",
                altText:""
            },
            {
                id:2,
                src:require("assets/img/kayb/image2.jpeg"),
                caption:"KayB",
                altText:""
            },
            {
                id:3,
                src:require("assets/img/kayb/image3.jpeg"),
                caption:"KayB",
                altText:""
            }
        ],
        music:[]
    },
];
export default data;